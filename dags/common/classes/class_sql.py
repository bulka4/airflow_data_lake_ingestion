...
class SQL:
    def __init__(
        ...
        if username == None and password == None:
            ...
            # connection_url = f"mssql+pyodbc://{username}:{password}@{server}/{database}?driver={driver.replace(' ', '+')}&Encrypt=yes&TrustServerCertificate=no&Connection+Timeout=30"
            # connection_url = (
            #     f"mssql+pyodbc://{username}:{password}@{server}:1433/{database}?"
            #     f"driver={driver.replace(' ', '+')}&Encrypt=yes&TrustServerCertificate=no&Connection+Timeout=30"
            # )
